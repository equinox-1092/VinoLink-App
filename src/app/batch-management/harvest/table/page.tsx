...
import { users, columnsHarvest } from "@/hooks/batch-management/useTableDownload";
...
export default function HarvestTable({ search }: HarvestTableProps) {
  const { t } = useTranslation();

  const tableColumns = React.useMemo<Column<UserType>[]>(() => {
    return columnsHarvest(t).map((col) => ({
      ...col,
      renderCell:
        col.uid === "name"
          ? (user: UserType) => (
