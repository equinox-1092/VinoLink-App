...
import { users, columnsRacking } from "@/hooks/batch-management/useTableDownload";
...
interface RackingTableProps {
  search: string;
}

export default function RackingTable({ search }: RackingTableProps) {
  const { t } = useTranslation();
  
  const tableColumns = React.useMemo<Column<UserType>[]>(() => {
    return columnsRacking(t).map(col => ({
      ...col,
